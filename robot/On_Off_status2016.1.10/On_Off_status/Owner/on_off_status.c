...
void EXTI15_10_IRQHandler(void)
{
	...
	if((EXTI->PR & EXTI_Line15) != (uint32_t)RESET)   // EXTI_GetITStatus(EXTI_Line15)
	{

		EXTI->PR = EXTI_Line15;      //EXTI_ClearITPendingBit(EXTI_Line15)
		if(MOSI_SWITCH[0])//主控发来的标志位
		{
			Motor_enc[0] = (float)Motor_Pos[4];
			Moto_Set_GSpeed(W_MOTOR5_OLD_ID,(int16_t)200);
		}
		if(MOSI_SWITCH[1])//当完成初始化后设定的限位标志，防止紧急事件
		{
			Moto_Stop(W_MOTOR5_OLD_ID);
		}
	...
	if((EXTI->PR & EXTI_Line10) != (uint32_t)RESET)  // EXTI_GetITStatus(EXTI_Line10)
	{
		...
		if(MOSI_SWITCH[0])//主控发来的标志位
		{
			MOSI_SWITCH[0] = 0x00;
			MOSI_STATUS[0] = 0x01;
			Motor_enc[1] = (float)Motor_Pos[4];
			Factor = (Motor_enc[0] - Motor_enc[1])/Length;
			MOSI_FACTOR.f32_data = Factor;
			Moto_Stop(W_MOTOR5_OLD_ID);
			Write_Database(W_STATUS_ID);
			Write_Database(BACK_FACTOR_ID);
		}
		if(MOSI_SWITCH[1])//当完成初始化后设定的限位标志，防止紧急事件
		{
			Moto_Stop(W_MOTOR5_OLD_ID);
